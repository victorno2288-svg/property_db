...
const SearchSuggestBox = ({ visible, onSelect, onClose, inputValue = '', activeFilters = {} }) => {
 ...
 .suggest-chip:hover {
 background: rgba(26,58,24,0.95); border-color: rgba(26,58,24,0.95); color: #fff;
 transform: translateY(-1px);
 }
 .suggest-row { cursor: pointer; padding: 10px 16px; display: flex; align-items: center; gap: 10px; font-size: 0.88rem; color: #1a1a1a; font-weight: 500; transition: background 0.12s; }
 .suggest-row:hover { background: rgba(26,58,24,0.08); }
 .suggest-section-title {
 font-size: 0.68rem; font-weight: 700; color: rgba(26,58,24,0.55);
 ...
 key={st.id}
 className="suggest-row"
 onClick={() => { onSelect(st.name); goSearch({ search: st.name }); }}
 >
 <span style={{
 ...
 onClick={() => goSearch({ search: st.name })}
 >
