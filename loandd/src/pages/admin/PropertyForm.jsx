...
function PropertyForm() {
  ...
  const addTransitToNearby = async () => {
    const pid = savedId || id;
    if (!pid) { setSaveMsg('⚠️ กรุณาบันทึกข้อมูลก่อนเพิ่มสถานที่'); setTimeout(() => setSaveMsg(''), 3000); return; }
            ...
            <div style={{ background: 'linear-gradient(135deg,#0d2347,#1a3c6e)', borderRadius: 14, padding: '18px 20px', marginBottom: 16, color: '#fff' }}>
              ...
              {existingDeeds.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.5)', marginBottom: 6 }}>📁 รูปโฉนดที่บันทึกไว้แล้ว ({existingDeeds.length} รูป):</div>
              ...
              {existingDeedUrl && existingDeeds.length === 0 && (
                <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: 10, padding: '8px 12px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.78rem' }}>
                  <a href={`${API}${existingDeedUrl}`} target="_blank" rel="noreferrer" style={{ color: '#a8f0c6', textDecoration: 'none' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                {deedSlots.map((slot, idx) => (
                  <div key={slot.slotId} style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: '12px', width: 'calc(50% - 6px)', minWidth: 220, boxSizing: 'border-box', border: '1px solid rgba(255,255,255,0.12)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgba(255,255,255,0.6)' }}>โฉนด #{idx + 1}</span>
                      {deedSlots.length > 1 && (
                        <button type="button" onClick={() => removeDeedSlot(slot.slotId)}
                          style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'rgba(255,100,100,0.7)', cursor: 'pointer', fontSize: '0.75rem', padding: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {slot.result && !slot.result.error && slot.result.filled.length > 0 && (
                          <div style={{ background: 'rgba(4,170,109,0.15)', border: '1px solid rgba(4,170,109,0.35)', borderRadius: 6, padding: '6px 8px' }}>
                            <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#a8f0c6', marginBottom: 4 }}>✅ {slot.result.filled.length} รายการ:</div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                              {slot.result.filled.map((item, i) => (
                                <span key={i} style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 3, padding: '1px 6px', fontSize: '0.62rem', color: 'rgba(255,255,255,0.85)' }}>✓ {item}</span>
                              ))}
                            </div>
                            {slot.result.fields && (
                        ...
                        {slot.result && !slot.result.error && slot.result.filled.length === 0 && (
                          <div style={{ fontSize: '0.68rem', color: '#ffe080' }}>⚠️ ไม่พบข้อมูล — ถ่ายให้ชัดขึ้น</div>
                        )}
                        {slot.result?.error && (
                          <div style={{ fontSize: '0.68rem', color: '#fca5a5' }}>❌ {slot.result.error}</div>
                        )}
                        {!slot.file && (
                ...
                <input
                  ...
                  placeholder={
                    !import.meta.env.VITE_GOOGLE_MAPS_KEY || import.meta.env.VITE_GOOGLE_MAPS_KEY === 'YOUR_GOOGLE_MAPS_API_KEY_HERE'
                      ? '⚠️ ยังไม่ได้ใส่ VITE_GOOGLE_MAPS_KEY ใน .env'
        ...
        {step === 3 && (
          <>
            ...
            <FormCard title="รายละเอียดทรัพย์" icon="fa-list-alt">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <FieldRow label="เฟอร์นิเจอร์">
                  <select value={form.condition_status} onChange={e => set('condition_status', e.target.value)} style={fieldStyle}>
                    ...
                    <option value="full">🛋️ ครบชุด (Fully Furnished)</option>
                  </select>
                </FieldRow>
              ...
              {/* VDO Tour */}
              <FieldRow label="VDO Tour (YouTube)" hint="วาง URL YouTube หรือ Google Drive เพื่อแสดง virtual tour">
                <div style={{ position: 'relative' }}>
                  <i className="fab fa-youtube" style={{ ...prefix, color: '#FF0000' }} />
                    ...
                    {existingImages.map(img => (
                      <div key={img.id} style={{ position: 'relative' }}>
                        ...
                        <button type="button" onClick={() => deleteExistingImage(img.id)}
                          style={{ position: 'absolute', top: -5, right: -5, background: '#e53e3e', color: '#fff', border: 'none', borderRadius: '50%', width: 22, height: 22, cursor: 'pointer', fontSize: '0.65rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          ✕
                        </button>
                      </div>
                    ))}
