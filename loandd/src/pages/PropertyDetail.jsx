...
function PropertyDetail() {
 ...
 // รองรับ YouTube / Facebook / direct video file
 const getVideoEmbed = (url) => {
 if (!url) return null;
 const ytId = getYoutubeId(url);
 if (ytId) {
 return { type: 'iframe', src: `https://www.youtube.com/embed/${ytId}?rel=0&modestbranding=1`, label: 'YouTube', icon: 'fab fa-youtube', color: '#FF0000' };
 }
 if (/facebook\.com|fb\.watch/.test(url)) {
 return { type: 'iframe', src: `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=0`, label: 'Facebook', icon: 'fab fa-facebook', color: '#1877F2' };
 }
 if (/\.(mp4|webm|mov)(\?|$)/i.test(url)) {
 return { type: 'video', src: url, label: 'ไฟล์วิดีโอ', icon: 'fas fa-play-circle', color: '#6aab62' };
 }
 return null;
 };
 ...
 <style>{`
 .photo-slide-wrap { position: relative; max-width: 1100px; margin: 0 auto; border-radius: ${T.radius}px ${T.radius}px 0 0; overflow: hidden; height: 520px; }
 ...
 ? (property.listing_type === 'rent' ? 'เช่าแล้ว' : 'ขายแล้ว')
 : 'จองแล้ว'}
 </div>
 ...
 {/* ===== PLANETVISION-STYLE HERO STRIP (Map Left + Stats Right) — ATTACHED TO PHOTO ===== */}
 {(() => {
 ...
 const stats = [
 { icon: 'fa-bed', label: 'Bedrooms', value: property.bedrooms > 0 ? property.bedrooms : null, suffix: '' },
 { icon: 'fa-bath', label: 'Bathrooms', value: property.bathrooms > 0 ? property.bathrooms : null, suffix: '' },
 { icon: 'fa-layer-group', label: 'Floors', value: property.floors > 0 ? property.floors : null, suffix: '' },
 { icon: 'fa-car', label: 'Parking', value: property.parking > 0 ? property.parking : null, suffix: '' },
 ].filter(s => s.value);
 return (
 <div style={{ padding: '0 16px' }}>
 <style>{`
 .pv-strip { display: grid; grid-template-columns: 1fr 1.6fr; gap: 0; border-radius: 0 0 ${T.radius}px ${T.radius}px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.1); border: 1px solid ${T.surfaceMid}; border-top: none; margin-top: -1px; max-width: 1100px; margin-left: auto; margin-right: auto; }
 .pv-strip-map { background: #1a1a1a; position: relative; min-height: 280px; }
 .pv-strip-map iframe { display: block; width: 100%; height: 100%; border: 0; filter: saturate(0.9) contrast(1.05); }
 .pv-strip-map-overlay { position: absolute; top: 14px; left: 14px; z-index: 2; display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 999px; background: rgba(0,0,0,0.65); backdrop-filter: blur(8px); color: #fff; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; font-family: 'Manrope', sans-serif; }
 .pv-strip-stats { background: #fafaf8; padding: 36px 32px; display: flex; flex-direction: column; justify-content: center; }
 .pv-strip-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 28px 16px; }
 .pv-stat { display: flex; flex-direction: column; gap: 8px; }
 .pv-stat-label { display: inline-flex; align-items: center; gap: 6px; font-size: 0.68rem; color: ${T.textMuted}; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; font-family: 'Manrope', sans-serif; }
 .pv-stat-label i { color: ${T.accent}; font-size: 0.75rem; }
 .pv-stat-value { font-size: clamp(1.4rem, 2.5vw, 1.9rem); font-weight: 800; color: ${T.text}; font-family: 'Manrope', sans-serif; letter-spacing: -0.02em; line-height: 1; }
 .pv-stat-unit { font-size: 0.85rem; color: ${T.textSoft}; font-weight: 500; margin-left: 4px; }
 @media (max-width: 768px) {
 .pv-strip { grid-template-columns: 1fr; }
 .pv-strip-map { min-height: 200px; }
 .pv-strip-stats { padding: 24px 20px; }
 .pv-strip-grid { grid-template-columns: repeat(2, 1fr); gap: 20px 12px; }
 }
 `}</style>
 <div className="pv-strip">
 <div className="pv-strip-map">
 <div className="pv-strip-map-overlay"><i className="fas fa-map-marker-alt" /> Location</div>
 <iframe title="location-mini-map" src={mapSrc} loading="lazy" allowFullScreen />
 </div>
 <div className="pv-strip-stats">
 <div className="pv-strip-grid">
 {stats.length > 0 ? stats.map((s, i) => (
 <div key={i} className="pv-stat">
 <div className="pv-stat-label"><i className={`fas ${s.icon}`} /> {s.label}</div>
 <div className="pv-stat-value">{s.value}<span className="pv-stat-unit">{s.label === 'Bedrooms' || s.label === 'Bathrooms' ? 'ห้อง' : s.label === 'Floors' ? 'ชั้น' : s.label === 'Parking' ? 'คัน' : ''}</span></div>
 </div>
 )) : (
 <div style={{ fontSize: '0.85rem', color: T.textMuted }}>ยังไม่มีข้อมูลจำเพาะ</div>
 )}
 {usableAreaText && (
 <div className="pv-stat">
 <div className="pv-stat-label"><i className="fas fa-vector-square" /> Usable Area</div>
 <div className="pv-stat-value" style={{ fontSize: 'clamp(1.05rem,1.8vw,1.25rem)' }}>{usableAreaText}</div>
 </div>
 )}
 {landAreaText && (
 <div className="pv-stat">
 <div className="pv-stat-label"><i className="fas fa-mountain-sun" /> Land Area</div>
 <div className="pv-stat-value" style={{ fontSize: 'clamp(1.05rem,1.8vw,1.25rem)' }}>{landAreaText}</div>
 </div>
 )}
 </div>
 </div>
 ...
 {/* ===== TITLE & BADGES (below PLANETVISION strip) ===== */}
 ...
 const hasDisc = (Number(property.is_discounted) === 1 || property.is_discounted === true)
 && Number(property.original_price) > 0
 && Number(property.original_price) > Number(property.price_requested);
 const discPct = hasDisc ? Math.round((1 - Number(property.price_requested) / Number(property.original_price)) * 100) : 0;
 return (
 <>
 {hasDisc && (
 <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 2, flexWrap: 'wrap' }}>
 <span style={{ color: T.textMuted, fontSize: '1rem', fontWeight: 500, textDecoration: 'line-through', fontFamily: T.fontBody }}>
 ฿{formatPrice(property.original_price)}
 </span>
 <span style={{
 background: 'linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)', color: '#fff',
 fontSize: '0.78rem', fontWeight: 800, padding: '4px 11px', borderRadius: 999,
 display: 'inline-flex', alignItems: 'center', gap: 5, letterSpacing: '0.02em',
 boxShadow: '0 3px 10px rgba(239,68,68,0.35)',
 }}>
 <i className="fas fa-fire" style={{ fontSize: '0.72rem' }} /> ลดราคา −{discPct}%
 </span>
 </div>
 )}
 <div style={{
 ...
 <div style={{
 width: 44, height: 44, background: '#f0f9e8',
 border: '1px solid #d4e8c8',
 borderRadius: '50%',
 display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
 }}>
 <i className="fas fa-train" style={{ color: '#3d7a3a', fontSize: '1.1rem' }} />
 </div>
 <div style={{ flex: 1, minWidth: 150 }}>
 <div style={{ color: '#888', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase', fontFamily: "'Manrope', sans-serif", fontWeight: 600, marginBottom: 3 }}>
 <i className="fas fa-map-marker-alt" style={{ color: '#C9A84C', marginRight: 5, fontSize: '0.65rem' }} />
 สถานีรถไฟฟ้าใกล้ที่สุด
 </div>
 <div style={{ color: '#1a1a1a', fontSize: '1.05rem', fontWeight: 700, fontFamily: "'Prompt', sans-serif", letterSpacing: '-0.01em' }}>
 {property.bts_station}
 </div>
 {property.bts_distance_km && (
 <div style={{ color: '#666', fontSize: '0.78rem', marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
 <span><i className="fas fa-route" style={{ color: '#3d7a3a', marginRight: 4, fontSize: '0.72rem' }} />{property.bts_distance_km} กม.</span>
 <span style={{ color: '#ccc' }}>·</span>
 <span><i className="fas fa-walking" style={{ color: '#3d7a3a', marginRight: 4, fontSize: '0.72rem' }} />~{Math.ceil(parseFloat(property.bts_distance_km) * 1000 / 80)} นาที</span>
 </div>
 )}
 </div>
 </div>
 ...
 {/* ===== MORTGAGE CALCULATOR (สำคัญกว่า แสดงก่อน) ===== */}
 {property.listing_type !== 'rent' && !!property.price_requested && (
 ...
 * ตัวเลขเป็นเพียงการประมาณการเบื้องต้น อัตราดอกเบี้ยและเงื่อนไขจริงขึ้นอยู่กับธนาคาร
 ...
 {/* ===== SIMILAR PROPERTIES (ทางเลือกอื่น แสดงล่างสุด) ===== */}
 ...
 {/* ===== VIDEO (รองรับ YouTube/Facebook/direct file) ===== */}
 {property.video_url && getVideoEmbed(property.video_url) && (() => {
 const embed = getVideoEmbed(property.video_url);
 return (
 ...
 style={{
 display: 'inline-flex', alignItems: 'center', gap: 6,
 marginTop: 12, fontSize: '0.78rem', color: embed.color,
 fontWeight: 600, textDecoration: 'none',
 }}>
 <i className={embed.icon} /> เปิดใน {embed.label}
 </a>
 </div>
 );
 })()}
 ...
 {/* Hashtags removed — user found them cluttering */}
 ...
 <div style={{ fontSize: '0.85rem', color: T.text, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
 {property.title}
 </div>
