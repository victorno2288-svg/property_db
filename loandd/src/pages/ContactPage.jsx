...
const ContactPage = () => {
  const [inquiryForm, setInquiryForm] = useState({ name: '', phone: '', email: '', message: '', type: 'ซื้อทรัพย์' });
    ...
    try {
      const res = await fetch(`${API_BASE}/api/inquiries`, {
        ...
        body: JSON.stringify({
          ...
          message: `[${inquiryForm.type}] ${inquiryForm.message.trim()}`,
        }),
      });
      ...
      if (res.ok) {
        ...
        setInquiryForm({ name: '', phone: '', email: '', message: '', type: 'ซื้อทรัพย์' });
      } else {
        setInquiryStatus('error');
  ...
  return (
    <div style={{ fontFamily: "'Sarabun', 'Noto Sans Thai', sans-serif", background: 'var(--surface, #FAF9F7)', minHeight: '100vh', paddingTop: '75px' }}>
      <Navbar />

      {/* Hero — Quiet Luxury */}
      <section style={{
        background: `linear-gradient(135deg, #3d7a3a 0%, #8BC683 60%, #A1D99B 100%)`,
